...
(define (do-something p lob)
  (cond
    [(empty? lob) '()]
    [(cons? lob)
     (cons (p (book-pages (first lob)))
           (do-something p (rest lob)))]))
...
(define (contains-page-ci? lob)
  (cond
    [(empty? (book-pages lob)) #f]
    [(string=? (first(book-pages lob)))#true]
    [else (contains-page-ci? lob)]))

(define (contains-page? lob)
  (do-something string-upper-case? lob))
...
(define (combine-pages lob)
 (cons (string-append (first (book-pages lob)) (second (book-pages lob))
 (combine-pages (rest lob)))))
...
(define (contains-page-ciw? lob)
  (cond
    [(empty? (book-pages lob)) #f]
    [(string=? (first(book-pages lob)))#true]
    [else (contains-page-ci? lob)]))

(define (contains-pagew? lob)
  (do-something string-upper-case? lob))
...
(define (contains-page-c2i? lob)
  (cond
    [(empty? (book-pages lob)) #f]
    [(string=? (first(book-pages lob)))#true]
    [else (contains-page-ci? lob)]))

(define (contains-paege? lob)
  (do-something string-upper-case? lob))
...
(define (contains-pwage-ci? lob)
  (cond
    [(empty? (book-pages lob)) #f]
    [(string=? (first(book-pages lob)))#true]
    [else (contains-page-ci? lob)]))

(define (containws-page? lob)
  (do-something string-upper-case? lob))
...
(define (do-something2 l lob)
  (cond
    [(empty? lob) #f]
    [(cons? lob)
     (cons (lob  (book-pages (first lob)))
           (do-something lob (rest lob)))]))
...
(define (contains-page-ci1? lob)
  (cond
    [(empty? (book-pages lob)) #f]
    [(string=? (first(book-pages lob)))#true]
    [else (contains-page-ci? lob)]))

(define (contains-pages? lob)
  (do-something string-upper-case? lob))
